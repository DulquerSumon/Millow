...
const Home = ({ home, account, provider, escrow, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    setBuyer(buyer);
    const hasBought = await escrow.approvals(home.id, buyer);
    setHasBought(hasBought);

    const seller = await escrow.seller();
    setSeller(seller);
    const hasSold = await escrow.approvals(home.id, seller);
  ...
  const buyHandler = async () => {
    ...
    const approveSale = await escrow.connect(signer).approveSale(home.id);
    await approveSale.wait();
  };
  const inspectHandler = async () => {
    const signer = await provider.getSigner();
    const update = await escrow
      .connect(signer)
  ...
  const sellHandler = async () => {
    ...
    const approveSale = await escrow.connect(signer).approveSale(home.id);
    await approveSale.wait();
    const finalizeSale = await escrow.connect(signer).finalizeSale(home.id);
